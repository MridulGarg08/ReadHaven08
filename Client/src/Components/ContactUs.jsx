import React from 'react'

function ContactUs() { 
  return (
    <>
    <div className='mx-auto justify-center max-w-screen-md mt-16'>
        <h4 className="font-bold text-lg text-center mb-2">Contact Us</h4>
        <p className="mb-4 text-center font-light  
                          text-gray-500 sm:text-xl"> 
                Got a issue? Want to send feedback? Let us know. 
            </p> 
        {/* <p className="py-4">Press ESC key or click on ✕ button to close</p> */}
        <form action="#"> 
                <div className="flex flex-row"> 
                    <div className="w-1/2 pr-2 "> 
                        <span className="block my-2 text-left  
                                          text-sm font-medium text-gray-900">
                            First Name 
                            </span> 
                        <input type="text" 
                               className="shadow-sm bg-gray-50 border 
                                          border-gray-300 text-gray-900  
                                          text-sm rounded-lg block w-full p-2.5" 
                               placeholder="Enter First Name"
                               required/> 
                    </div> 
                    <div className="w-1/2 pl-2"> 
                        <span className="block my-2 text-left text-sm  
                                          font-medium text-gray-900"> 
                            Last Name 
                        </span>
                        <input type="text" 
                               className="shadow-sm bg-gray-50 border  
                                          border-gray-300 text-gray-900  
                                          text-sm rounded-lg block w-full p-2.5"
                               placeholder="Enter Last Name"/> 
                    </div> 
                </div> 
                <div> 
                    <span className="block my-2 text-left text-sm  
                                      font-medium text-gray-900"> 
                        Your email 
                    </span>
                    <input type="email" 
                           className="shadow-sm bg-gray-50 border  
                                      border-gray-300 text-gray-900  
                                      text-sm rounded-lg block w-full p-2.5" 
                           placeholder="abc@gmail.com" 
                           required /> 
                </div> 
                <div> 
                    <span className="block my-2 text-left  
                                      text-sm font-medium text-gray-900"> 
                        Subject 
                    </span> 
                    <input type="text" 
                           className="block p-3 w-full text-sm  
                                      text-gray-900 bg-gray-50 rounded-lg  
                                      border border-gray-300 shadow-sm "
                           placeholder="What issue/suggestion do you have?" 
                           required /> 
                </div> 
                <div > 
                    <span className="block my-2 text-left  
                                      text-sm font-medium text-gray-900 "> 
                        Your message 
                    </span>
                    <textarea rows="6" 
                              className="block p-2.5 w-full text-sm  
                                         text-gray-900 bg-gray-50 rounded-lg  
                                         shadow-sm border border-gray-300 " 
                              placeholder="Query/Suggestion..."/> 
                </div> 
                <button type="submit" 
                        className="mt-2 p-2 float-right text-white   
                                   rounded-lg border-green-600  
                                   bg-green-600 hover:scale-105"> 
                  Send message 
                </button> 
            </form> 
      </div>
      </>
  )
}

export default ContactUs